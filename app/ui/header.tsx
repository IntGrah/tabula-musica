...
function Logo({ scale }: { scale: number }) {
    return (
        <div className="text-center">
            ...
            <hr
                className="m-auto w-64 border-violet-900"
                style={{
...
function NavLink({
    ...
    return (
        <Link
            ...
            href={href}
        >
            <span className="font-serif tracking-wider text-cyan-800 group-hover:text-cyan-600">
                {children}
            </span>
...
function Search() {
    return (
        <form className="h-12 py-1 absolute right-4">
            <input
                className="h-full px-4 w-20 text-cyan-800 bg-transparent outline-none rounded transition-all border border-transparent focus:border-gray-400 focus:w-32 placeholder:text-gray-500"
                placeholder="Search"
            />
        </form>
    );
